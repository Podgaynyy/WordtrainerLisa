...
Config.set('graphics', 'height', 500) #make size of mobile screeen, but resizable for using different phones
...
liste = [] #here create list in first of words in first screen for using in second screen
...
class MainScreen(Screen):
    def __init__(self):
        ...
        self.wort = TextInput() #for create a words
        bl.add_widget(self.wort)
        bl.add_widget(Label(text="Bedeutung")) #what a word means
    ...
    def erschaffeWoerterbuch(self, *args):    # Aufbau eines fremdsprache-deutschen Wörterbuchs
        ...
        if self.wort.text == '': #stop creating, if user forgot a word
            datei.close()
        elif self.wort.text == ' ':
            datei.close()
        else:
            datei.write(self.wort.text + " " + self.bedeutung.text + "\n") #create in a line two words
            # 1st a word and second is a meating
            self.wort.text = "" #clear input area
            self.bedeutung.text = ""


    def getListe(self): #create list of words from document by switching to another screen
        global liste
        datei = open('woerterbuch.txt', 'r', encoding='utf-8') #take a words from file
    ...
    def to_second_scrn(self, *args): #switch a screen
...
class SecondScreen(Screen):
    ...
    # on this screen, I do everything the same as on the main screen to be able to switch back and forth
        ...
        # nehme ich Anchorlayout als Grundwerkstoff
        ...
        self.title2 = Label(text="Wörte lernen", font_size=60, size_hint=(1,.7))
        ...
        self.abfrage = Label(font_size=60, size_hint = (1,.3))  # Wird Woerte abgefragt
        ...
        self.blDown = BoxLayout(orientation='vertical', size_hint = (0.8, 0.8))
